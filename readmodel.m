...
% GDP weight mining sector
p.w_mine = 0.15;

% Potential output growth
p.ss_DLA_GDP_BAR = [5.6]; %2.5;
p.ss_DLA_GDP_MINE_BAR = [0]; %2.5;
p.ss_DLA_GDP_OTH_BAR = [4]; %2.5;


% Domestic inflation target
p.ss_D4L_CPI_TAR = 6; %2; 
...
p.ss_DLA_CPI_RW = 3.0; %2;

% Level of foreign real interest rate
p.ss_RR_RW_BAR = 1.35; %0.75;
...
% output persistence;
p.b1 = 0.7; %0.8; % b1 varies between 0.1 (extremely flexible) and 0.95(extremely persistent)
p.b1_mine = 0.7;
p.b1_oth = 0.7;

% policy passthrough (the impact of monetary policy on real economy); 
p.b2 = 0.2; %0.3; % b2 varies between 0.1 (low impact) to 0.5 (strong impact)
p.b2_oth = 0.2;

% the impact of external demand on domestic output; 
p.b3 = 0.5; %0.5; % b3 varies between 0.1 and 0.7
p.b3_mine = 0.5;
p.b3_oth = 0;


% the weight of the real interest rate and real exchange rate gaps in Monetary Conditions Index;
p.b4 = 0.4; %0.7; % b4 varies from 0.3 to 0.8
...
p.a2 = 0.30; %0.2; % a2 varies between 0.1 (a flat Phillips curve and a high sacrifice ratio) to 0.5 (a steep Phillips curve and a low sacrifice ratio)

% the ratio of domestic costs in firms' aggregate costs
p.a3 = 0.5; %0.7; % a3 varies between 0.9 (for a relatively more closed economy) to 0.5 (for a relatively more open economy)
...
% policy persistence; 
p.g1 = 0.8; % g1 varies from 0 (no persistence) to 0.8 ("wait and see" policy)

% policy reactiveness: the weight put on inflation by the policy-makers 
p.g2 = 1.0; %0.5; % g2 has no upper limit but must be always higher then 0 (the Taylor principle)

% policy reactiveness: the weight put on the output gap by the policy-makers 
p.g3 = 0.1; % g3 has no upper limit but must be always higher then 0
...
p.rho_DLA_GDP_MINE_BAR = 0.8;
p.rho_DLA_GDP_OTH_BAR = 0.8;
p.rho_RR_BAR      = 0.8;
p.rho_RR_RW_BAR   = 0.8;
